...
def safe_predict(features, action='predict', timeout=5):
    ...
    try:
        payload = json.dumps({'features': features, 'action': action})
...
def detect_fraud(user_id, card_no, amount, location, ip_address, user_data, behavior_data, cursor):
    ...
    # fraud_score is 0-1 internally; convert to percentage for thresholds/output
    fraud_score_pct = fraud_score * 100

    if fraud_score_pct <= 20:
        status = 'Approved'
    ...
    return {
        'status': status,
        'fraud_score': round(fraud_score_pct, 2),
        'ml_score': round(ml_score * 100, 2),
        'bla_score': round(bla_score * 100, 2),
        'method': method,
        'message': message
    }
